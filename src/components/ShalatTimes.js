...
export default function ShalatTimes() {
  ...
  const fetchShalat = useCallback(async (prov, city) => {
    setLoading(true);
    setError(null);
    try {
      ...
      if (res.data.code === 200) {
        ...
        // API Shalat returns 'date' field YYYY-MM-DD in its jadwal array usually
        // Based on doc, let's try to find by date
        const schedule =
          data.jadwal.find((item) => item.date === todayStr) ||
          data.jadwal[today.getDate() - 1] || // Fallback by day index
          data.jadwal[0];
    ...
    const init = async () => {
      ...
      if (saved) {
        try {
          ...
          await fetchShalat(currentLoc.provinsi, currentLoc.kabkota);
          return;
        } catch (e) {
          ...
          async (pos) => {
            try {
              ...
              if (matchedProv) {
                ...
                if (matchedCity) {
                  await fetchShalat(matchedProv, matchedCity);
                  return;
                }
              }
  ...
  const handleCityChange = (e) => {
    ...
    fetchShalat(location.provinsi, city);
    setIsEditing(false);
  };
        ...
        async (pos) => {
          try {
            ...
            if (matchedProv) {
              ...
              if (matchedCity) {
                fetchShalat(matchedProv, matchedCity);
                setIsEditing(false);
              } else {
                setLocation({
